...
if($action=='addusers'){
	...
	$name=$_POST['name'];
	$acadimic_id=$_POST['acadimic_id'];
	$id_password=$_POST['id_password'];
	$email=$_POST['email'];
	$password=$_POST['password'];
	$role=$_POST['role'];
	 
	$sql="INSERT INTO `register`( `name`, `acadimic_id`, `id_password`, `email`,`password`,`role`) VALUES('$name','$acadimic_id','$id_password','$email','$password','$role')";
	$result=$conn->query($sql);
	if($result===true){
		$res['error']=false;
